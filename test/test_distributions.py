...
def test_simple_posterior():
    ...
    assert math.isclose(
        result, math.log(1 / 4 / math.pi)
    ), f"Posterior does not have the correct value, expected {math.log(1 / 4 / math.pi)}, got {result}"

    assert (
        simple_posterior(0, alpha=-100, beta=1) < -10e100
    ), "Posterior does not behave correctly for out of range parameters"

    assert (
        simple_posterior(0, alpha=0, beta=1e4) < -10e100
    ), "Posterior does not behave correctly for out of range parameters"
...
def test_intensity_posterior():
    ...
    assert (
        intensity_posterior(0, np.log(0.01), alpha=-100, beta=1, i_0=1) < -10e100
    ), "Posterior does not behave correctly for out of range parameters"

    assert (
        intensity_posterior(0, np.log(0.01), alpha=0, beta=1e4, i_0=1) < -10e100
    ), "Posterior does not behave correctly for out of range parameters"

    assert (
        intensity_posterior(0, np.log(0.01), alpha=-100, beta=1, i_0=1e9) < 10e-100
    ), "Posterior does not behave correctly for out of range parameters"
